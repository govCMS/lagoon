...
$settings['fast404_exts'] = '/^(?!robots)^(?!\/system\/files).*\.(?:png|gif|jpe?g|svg|tiff|bmp|raw|webp|docx?|xlsx?|pptx?|swf|flv|cgi|dll|exe|nsf|cfm|ttf|bat|pl|asp|ics|rtf)$/i';
...
if (getenv('GOVCMS_FAST404_EXT')) {
  // Allow sites to override the regex pattern for the fast404_ext if required.
  $settings['fast404_exts'] = getenv('GOVCMS_FAST404_EXT');
}

if (getenv('GOVCMS_FAST404_WHITELIST')) {
  // Allow sites to override the whitelist.
  $settings['fast404_whitelist'] = explode(',', getenv('GOVCMS_FAST404_WHITELIST'));
}
